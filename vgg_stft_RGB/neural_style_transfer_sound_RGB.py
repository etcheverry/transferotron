...
base_stft = librosa.stft(base_sound)
style_stft = librosa.stft(style_sound)
...
def deprocess_image(x, phases, width, height, maximum):
    ...
    for i in range(width):
        for j in range(height):
            ...
            g = x[i][j][1] + 116.779
            b = x[i][j][2] + 123.68
            out[i][j] = cmath.rect(rgb_to_norm(b, g, r)*maximum, phases[i][j])
    return out

base_array, base_phases, base_maximum = preprocess_image(base_stft)
style_array, style_phases, style_maximum = preprocess_image(style_stft)
...
'''
model = VGG19_SOUND(input_tensor=input_tensor,
                    weights=None, include_top=False)
model.load_weights('./models/vgg_sound.h5', by_name=True)
'''
print('Model loaded.')
...
for i in range(iterations):
    ...
    img = deprocess_image(x.copy(), base_phases, img_nrows, img_ncols, 1)
    print(img)
